...
<body style="background-color:#F6E6C5;overflow-x: hidden; ">
 ...
 <img src="img/bg6.png" id="Card1">
<div id="height0">
  <img src="img/ha.png" id="ha">
  <!--<img src="img/burger.png" id="burger">-->
  <div>
          <p class="pro3 pro4">HOT_</p> 
  </div>

<%
					sql = "Select id , kinds , price , stock , photo FROM product ORDER BY selled DESC ";
                    //SQL語法，選擇要顯示的欄位。
                    ResultSet rec = con.createStatement().executeQuery(sql);

					boolean yesno = false ;
					int howmany = 1, count = 0,linereturn = 0 ;
					int printt = 1 ;
					int rank = 0 ;
                    while ( rec.next() && rank < 3 ) 
					{
						rank++ ;
						...
						String pid = rec.getString(1);
							...
							out.println("<div class='div_shadow"+howmany+" div_shadow'>") ;
										
							out.println("<div id='bgh1'>");
							out.println("</div>");
						sql="select email from comsumer where email='" + session.getAttribute("email")+"'"; 
						ResultSet ros=con.createStatement().executeQuery(sql); 
						sql="select email from manager where email='" + session.getAttribute("email")+"'"; 	
						ResultSet ross=con.createStatement().executeQuery(sql); 
						if ( ros.next()){
						out.println("<a href='showhand.jsp?page=" + pid + "'class='hvr-buzz'><img src='img/"+photostring+"' id='h1' ></a>") ;
						}
						else if ( ross.next() || session.getAttribute("email")==null ){
						out.println("<a class='hvr-buzz'><img src='img/"+photostring+"' id='h1' ></a>") ;
						}
							out.println("<font id='h1font'>");
							out.println( kindsstring + "$" + pricestring ) ;
							out.println("</font>");
							out.println("<font id='nofont'>");
%>
